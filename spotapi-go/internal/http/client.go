...
// NewClient creates a Client configured with the given TLS profile, optional proxy, and retry policy.
// 
// The proxy parameter, when non-empty, should be host[:port]; it will be used with an "http://" scheme.
// autoRetries is the configured number of retry attempts; the Client's actual attempt count is autoRetries + 1.
// Returns an error if the underlying TLS-capable HTTP client cannot be created.
